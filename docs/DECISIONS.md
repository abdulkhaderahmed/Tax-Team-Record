...
## D-025 · 2026-07-25 · Matter is a first-class record, and deliberately thin
`Matter` exists in the database with its own reference series (`MAT-<year>-<nnn>`, allocated per organisation), create and resolve server actions behind `record:write`, and audit events attributed to the authenticated user. It carries only what makes a matter a matter — the decision question, type, owner, status and target date — and **links** existing Documents, Actions, Obligations and Approvals rather than duplicating them.
**Counterfactual:** projecting "matter" as a view over a Document plus its review items was rejected — it cannot represent work that starts from a business event rather than a document, which is most transaction work. Duplicating registers onto the matter was also rejected; that is how the one-spine rule in D-016 gets broken. `decisionQuestion` is a required field for the same reason a decision window must state its no-action outcome: a matter with no decision to reach is a folder.
**Found on the way:** the entire matter concept previously existed only as static demo components. "Start a matter" linked to a read-only sample and "Add work" had no click handler. 33 buttons across the four worked-example surfaces are still non-functional by design — they illustrate, they do not operate.
