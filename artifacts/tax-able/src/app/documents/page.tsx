        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20 }}>
          <div>
            ...
            <p style={{ margin: 0, color: "var(--ink-secondary)", fontSize: 13 }}>
              ...
              {totalHealthFlags > 0 && (
                <span style={{ marginLeft: 10, color: "var(--due-soon)" }}>· {totalHealthFlags} health flag{totalHealthFlags !== 1 ? "s" : ""} detected</span>
              )}
            </p>
          </div>
                ...
                <tbody>
                  {documents.map((doc) => (
                    <tr key={doc.id} style={doc.status === "Archived" ? { opacity: 0.55 } : {}}>
                      ...
                      <td style={{ color: "var(--ink-secondary)", fontSize: 12 }}>{bytes(doc.fileSize)}</td>
                      <td style={{ color: "var(--ink-secondary)", fontSize: 12, whiteSpace: "nowrap" }}>{fmt(doc.uploadedAt)}</td>
