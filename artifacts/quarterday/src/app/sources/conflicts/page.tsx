...
function SeverityBadge({ severity }: { severity: string | null }) {
  if (!severity) return <span style={{ color: "#9ca3af" }}>—</span>;
  const cls =
      ...
      <div style={{ padding: "28px 32px" }}>
        ...
        <div className="tab-bar" style={{ marginBottom: 24 }}>
          ...
          <Link href="/sources/conflicts" className="tab tab-active">
            Conflicts{openCount > 0 && <span style={{ marginLeft: 6, background: "#fef2f2", color: "#b91c1c", borderRadius: 10, padding: "1px 7px", fontSize: 11 }}>{openCount}</span>}
          </Link>
        </div>
                ...
                <tbody>
                  {conflicts.map((c) => (
                    <tr key={c.id}>
                      ...
                      <td>{c.entity?.legalName || <span style={{ color: "#9ca3af" }}>—</span>}</td>
                      ...
                      <td>{c.reviewOwner || <span style={{ color: "#9ca3af" }}>—</span>}</td>
                      <td>
